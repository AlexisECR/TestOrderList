...
const productosConvertidos = productosDummy.products.map((producto) => {
    return {
...
productosConvertidos.forEach((producto: ProductoDummy) => {

    let article: HTMLDivElement = document.createElement('div');
    article.setAttribute('class','article');
    articleImage(producto.images[0], article);
    articleDescription(producto.title,producto.category, article);
    articlePrice(producto.price, producto.stock, article);
    addArticle(article);

});

function articleImage(img:string, article:HTMLDivElement): void{

    let articleImage: HTMLDivElement = document.createElement('div');
    articleImage.setAttribute('class','article-image');

    let image: HTMLDivElement = document.createElement('img');
...
function articleDescription (title:string, category:string, article:HTMLDivElement):void {
    ...
    articleCategory.innerText = category;
...
function articlePrice (price:number,existenceStock:number, article:HTMLDivElement):void {
    ...
    typeStock(existenceStock,stock);
...
function addArticle (article: HTMLDivElement) {
    document.getElementById("listArticles")?.appendChild(article);
}

function typeStock(amountStock:number,stock:HTMLSpanElement){
    if(amountStock >= 50)
    {
    ...
    {
        stock.setAttribute('class','lowstock');
        stock.innerText = "LOWSTOCK";
    }
    else
    {
